...
function usuarioPermitido(userId) {
  return usuariosPermitidos.includes(userId);
}
...
bot.command(['Rut', 'rut', 'RUT'], (ctx) => {
  ...
  if (usuarioPermitido(ctx.from.id)) {
    ...
    ctx.reply('Ingrese el RUT del paciente (sin puntos y con guión):');
    //"Espera la entrada del RUT del usuario dentro del manejador de comandos."
    bot.use((ctx, next) => {
      var RUTa = ctx.message.text;
      console.log(RUTa);

      if (!RUTa) {
        console.log('RUT no válido');
      } else {
        // Procesando un RUT ingresado    
        ...
        //fin 
        ctx.state.users = 75;
        next(ctx); //next se pasa porque podemos modificar los datos
      }
